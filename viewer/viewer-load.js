  ...
  return json.replace(/("(\\u[a-zA-Z0-9]{4}|\\[^u]|[^\\"])*"(\s*:)?|\b(true|false|null)\b|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?)/g, function(m) {
    ...
    if (/^"/.test(m)) {
      if (/:$/.test(m)) {
        ...
        if (m.length > 300) {
          var id = 'trunc-' + (tId++);
          var preview = escapeHtml(m.slice(0, 80)) + '…"';
          var full = escapeHtml(m);
...
function loadJSONL(text, fileName) {
  ...
  try {
    ...
    document.getElementById('stats').innerHTML =
      'File: ' + escapeHtml(fileName) + ' | Raw: ' + tree.totalRawNodes +
    ...
    document.getElementById('tree').innerHTML = '<div class="empty-state" style="color:#f44336;">Error: ' + escapeHtml(err.message) + '<br><pre style="text-align:left;margin-top:10px;font-size:11px;color:#999;">' + escapeHtml(err.stack || '') + '</pre></div>';
  }
}
