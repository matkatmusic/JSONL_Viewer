...
function renderTree(tree) {
  ...
  function label(node) {
    ...
    return r + side + ': ' + escapeHtml(node.contentSummary.slice(0,80));
  }

  function deriveRoleClass(node) {
  ...
  function renderDiff(data) {
    var h = '<div class="diff-view"><div class="diff-header">' + escapeHtml(data.filePath.split('/').pop()) + ' (' + data.opType + ')</div>';
    if (data.opType === 'create' && data.content) {
      h += '<div class="diff-hunk">';
      var lines = data.content.split('\n');
      for (var i = 0; i < lines.length; i++) {
        h += '<div class="diff-line diff-add"><span class="diff-text">+' + escapeHtml(lines[i]) + '</span></div>';
      }
      h += '</div>';
    } else {
      for (var hi = 0; hi < data.hunks.length; hi++) {
        ...
        for (var li = 0; li < hunk.lines.length; li++) {
          ...
          h += '<div class="diff-line ' + cls + '"><span class="diff-text">' + escapeHtml(pfx + txt) + '</span></div>';
        }
        h += '</div>';
      }
    }
  ...
  function collectFlatNodes(node) {
    var out = [];
    function walkDirectory(n) { out.push(n); for (var i = 0; i < n.visibleChildren.length; i++) walkDirectory(n.visibleChildren[i]); }
    var ak = node.attachKids || [];
    for (var i = 0; i < ak.length; i++) walkDirectory(ak[i]);
    return out;
  }

  function renderPlumbing(node, depth) {
    var ac = ap[node.uuid] ? 'active' : 'orphaned';
    var rc = deriveRoleClass(node);
    var dc = node.hasDiff ? ' has-diff' : '';
    var fn = node.diffFile ? node.diffFile.split('/').pop() : '';
    var lb = node.hasDiff
      ? escapeHtml(node.contentSummary.slice(0,40)) + ' <span class="diff-badge">&Delta; ' + escapeHtml(fn) + '</span>'
      : label(node);
    var af = collectFlatNodes(node);
    var hasTool = af.length > 0;
    var h = '<div class="node ' + ac + ' ' + rc + dc + (hasTool ? ' has-tool-children' : '') + '" style="margin-left:' + (depth*INDENT_PX) + 'px;" data-uuid="' + node.uuid + '"><span class="toggle">[+]</span> <span class="label">' + lb + '</span>' + renderActionButtons(node) + '</div>\n';
    if (node.diffData) {
  ...
  function renderSingleNode(node, depth, plumbing) {
    plumbing = plumbing.concat(collectFlatNodes(node));
    var ac = ap[node.uuid] ? 'active' : 'orphaned';
    var rc = deriveRoleClass(node);
    ...
    var di = containsDiff ? ' <span class="diff-badge">&Delta; ' + escapeHtml(diffFiles.join(', ')) + '</span>' : '';
    ...
    h += '<span class="toggle">[+]</span> <span class="label">' + label(node) + '</span>' + di + renderActionButtons(node) + '</div>\n';
    if (hasTool) {
      h += '<div class="tool-children" style="display:none;margin-left:' + ((depth+1)*INDENT_PX) + 'px;">';
