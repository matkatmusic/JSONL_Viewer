...
## What Remains
1. **Recon S2 vocabulary.** Write a Node scan over the S2 JSONL producing per-record-type field inventories — mirror `recon/06`–`recon/09`. S2 JSONL, via the in-repo symlink (preferred): `scenarios/executed/s2-move-file/1e82511e-05a3-4712-9a95-206b24128694.jsonl`; absolute equivalent: `/Users/matkatmusicllc/Desktop/claude code src/RevEng/plans/scenarios/executed/s2-move-file/1e82511e-05a3-4712-9a95-206b24128694.jsonl`. Run it in the MAIN agent unless you've confirmed subagents can follow the symlink (see Context).
...
- **Scenario JSONLs live OUTSIDE this worktree** (the Desktop path). There is now an in-repo symlink `scenarios/` → `/Users/matkatmusicllc/Desktop/claude code src/RevEng/plans/scenarios` (gitignored as `/scenarios`; machine-specific absolute target), so the data is reachable at `scenarios/executed/<scenario>/<uuid>.jsonl`. The MAIN agent reads it fine. **Whether sandboxed subagents can follow the symlink is UNVERIFIED** — some sandboxes resolve the link to its real target and still block reads outside the worktree. Before fanning out subagents to read scenario data, TEST IT with one cheap subagent read; if blocked, keep the recon scan and the whole TDD loop in the MAIN agent (as for S1).
