# Plan — Task 337: UI component glossary

## Goal
Create `jfred/docs/ui-component-glossary.md` mapping the loose names the user uses for
Layer 1 page components to their canonical name, owning `jfred/webapp/layer1-*.ts` module,
DOM id / CSS class, and a one-line purpose. Add a pointer in the project `CLAUDE.md`.

Scope: disambiguation only. Each entry under 30 words. No behavior/implementation prose.

## Step 1 — write `jfred/docs/ui-component-glossary.md`

Exact content (identifiers already verified against the source):
...
## Step 2 — pointer in project `CLAUDE.md`

Append a short section to `/Users/matkatmusicllc/Desktop/claude code src/RevEng/CLAUDE.md`
so agents load the glossary when the user names a component. Add:
...
## Verification
- File exists at `jfred/docs/ui-component-glossary.md` and renders as a table.
- CLAUDE.md pointer present.
- No typecheck impact (docs only). Nothing to run.

## Notes for implementer
- Docs-only change; do not touch any `.ts`. No tests.
- Identifiers above were read from source — copy them verbatim, do not re-derive.
