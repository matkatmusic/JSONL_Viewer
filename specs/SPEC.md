...
- Tasks: #183, #184
- Status: done (#183 done — /api/file-ladder endpoint (viewer_api_ladder.ts, wired at viewer_server.ts:187) + /app/debug.html skeleton with file selection and deep-link, committed with tests/viewer_api_ladder.test.ts and tests/debug-app.test.ts; #184 done 2026-07-25 — webapp/debug-app.ts renders per revision the kind + changeId as source attribution, the timestamp, `unrecoverable.reason` as the conflict note, and the seed hash on `gitBase:<hash>:<target>` revisions, verified against the real claude-data 18-revision `control` and 10-revision `iter2b` plate_cli.py ladders: li-count = revision count, seed hash = expected commit, no undefined/null/NaN/[object Object], every timestamp parseable. Known limit: `FileRevision` carries no multi-source provenance field, so source attribution is kind + changeId; the conflict-note path is exercised only by the synthetic fixture because the real captures had zero unrecoverable revisions)
...
- Tasks: #186, #187
- Status: open (#186 done 2026-07-25, staged — jfred/scripts/per_file_sweep.ts + jfred/src/per_file_sweep_report.ts: one process, one records array, N targets through the task-182 fast path; resumable JSONL log + markdown table with an `ok`/`gaps`/`endpoint-miss`/`none` verdict per candidate, recipe in plans/166-per-file-target.md. #187 done 2026-07-25 — all 68 M files swept in 904 s (827 s of that a one-time warm-up on candidate 1; the other 67 cost 77 s combined), 0 unrecoverable revisions. Raw: 22 fully ok, 46 `diskMatched=false`, 2 `baselineBlobMatched=false`, 28 one-revision ladders. Triaged, the real defect list is 10 files, not 46: `diskMatched` is a ONE-WAY oracle, and content-hashing all 11,748 file-history snapshots shows 35 of the 46 misses have current bytes present in NO recorded source, so no engine could satisfy them; 27 of the 28 one-revision ladders are the engine being correct, those files having zero write-class tool_uses across the 156 transcripts. Full detail in plans/166-per-file-target.md, section "Task 187 — the M-file sweep (68 files)")
...
- Tasks: #188, #189
- Status: open (#188 done 2026-07-25 — the loop ran end-to-end on plate_cli.py in a scratch clone: bounded reconstruct (12 s) → commit the chunk's last revision at its own committer instant → re-seed (48 s, 13 new revisions). TWO findings block leaning on it: the CLI never prunes pre-baseline records, so a re-seed does not move the window (task 223), and a mid-window seed emits a non-monotonic ladder with a duplicated snapshot revision (task 224) — a control run at the same bound against the original baseline recovered the same history. Recipe + measurements in plans/166-per-file-target.md. Both blockers CLOSED 2026-07-25: #223 added the `--no-pre-baseline` CLI flag (the CLI equivalent of the viewer's "No" to the pre-baseline question) — on real data the mid-window re-seed ladder went 20 → 10 revisions and 66 s → 39 s, now starting exactly at the seed instant, and the duplicated `@v2`/`@v1` snapshot revisions #224 reported disappeared with it; #224 added `clampSeedBetweenPreviousAndEdit` in jfred/src/reconstruction_sidecar.ts, giving a backup-sourced seed a two-sided placement window `[previousEventTime, editTime)` and deleting `seedBeforeEdit` from jfred/src/reconstruction_reseed.ts — the guarantee is NON-DECREASING, not strictly increasing (the seed pins to equal the previous instant); verified on real data at 10/10 checks, exactly one timestamp re-stamped per run, no revisions added or dropped, no changeId churn, final content byte-identical to control. With #187 also done, the S11 phase gate is met and #189 is unblocked)
