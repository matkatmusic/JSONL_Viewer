...
- Tasks: #230, #231, #232, #233, #234, #235, #236, #237, #238, #239, #240, #241, #242, #243, #244, #245, #246, #247, #248, #249, #250, #251, #252, #253, #254, #255, #256, #257, #259, #260, #261, #262, #263, #264, #265, #266, #267, #268, #269, #270, #271, #272, #273, #274, #275, #276, #277, #278, #279, #280, #281, #282, #283, #284, #285, #286, #287, #288, #289, #294, #298, #299, #300, #301, #302, #303, #304, #305, #306, #321, #322
...
- **Duplicate-snapshot policy across configured File History Snapshot folders (#301, user-decided 2026-07-28).** The picker (#296) stores an ORDERED list of snapshot folders; precedence is that order — the FIRST configured root wins, and the picker order IS the precedence order. When two roots hold the same `<sessionId>/<hash>@vN` with DIFFERENT bytes, the first root's blob is used and the conflict is reported to the health sink (the #176 `noteJoinedPathConflicts` idiom); identical bytes in both roots are a silent no-op, and a blob missing in an earlier root falls through to the next. Implemented lazily on purpose: nothing consumes the folder list yet (Layer 1 reads no file-history), so the whole policy is one tested helper — `resolveBlobAcrossRoots` in `jfred/src/reconstruction_multi_root_blobs.ts`, beside the single-root sidecar reader — for the Layer 2 snapshot consumer to call, plus one hint line in the picker dialog ("earlier folders win on conflict").
- **Layer 1 close-out batch, user-verified DONE 2026-07-28 (#294, #299, #300, #301, #302, #303, #304, #305, #306, #321, #322).** NOTE: numbers 299–310 were INDEPENDENTLY reused for the Layer 2 mockup tasks on the parallel branch; this batch's two new bug tasks were renumbered 309→#321 and 310→#322 at merge time, and the follow-ups below carry merge-time numbers. All eleven closed in one parallel-worktree pass and merged to jfred@layer-1-milestone: image drawer with header zoom + binary byte route (#299, 43452b1), two-node lane diff reusing the Revision Viewer renderer lifted to `webapp/diff-render.ts` with syntax-highlighted diff lines and patch export (#305, 6b7b891+3df4ae9 — which also closed #294), expanded-row re-layout via `extraGapPx` per the signed-off mockup (#300, 1ede4e9), millisecond ruler labels after the #306 diagnosis proved the geometry honest and the centisecond label the lie (6deb8e0+49029eb), Cancel-with-confirm on an abortable NDJSON view stream (#302, b30b334), per-ladder resolve counter (#303, a0db493), per-file JSONL scan counter with a truthful empty-state (#304, 3d80f8b), the `markMultiEventTicks` O(rows×bubbles) render stall fixed by a snapshot + chunked "drawing the timeline" render (#321, e0c45a2), and the `.loadbar[hidden]` display rule (#322, 15770e4). Follow-ups filed, not blocking: #319 (split-view ± markers), #320 (full-content as a diff-preserving toggle), #323/#324 (drawer node-cycling arrows). #319, #320 and #323 closed 2026-07-29, user-verified (jfred a2d1efc, RevEng b1a8421); #324's two-arrow-pair header design settled by the user the same day and closed user-verified later that day along with #325 and #327 (jfred c253307, RevEng 5d3659a); #326's Show-Only-Selected first pass shipped in the same commits; its empty-intersection fix (undefined = no filter, empty intersection = empty stage) landed at jfred 3c060e0 and #326 closed user-verified 2026-07-29.
...
- Status: #299-#307 DONE 2026-07-27 (RevEng 66dd668, jfred 825f36b), signed off by the user and covered by 43 headless checks in `jfred/scripts/visual/mockup.ts` (`npm run visual:mockup`). #308/#309 also DONE 2026-07-27 (RevEng 7498bc0, jfred 64310f1): a bubble click flashes every JSONL that touched that file, leading with the session nearest the clicked instant, and the JSONLs pane has a customTitle search box that filters the list without filtering the timeline — 9 further checks in `jfred/scripts/visual/mockup-checks-nav.ts`. The build-out this mockup gates is tasks **#310-#318**, created 2026-07-27: engine #310 (snapshot discovery per owning session, reusing `collectSnapshotBeaconNodes` from S4/#201), #311 (session metadata + `titleInEffectAtLine`), #312 (snapshots on the wire, ruler and the shared axis), #313 (`/api/layer1-file` third form for snapshot bytes); page #314 (live `[2]`, `[3]` dropped), #315 (📸 nodes on the bubbles), #316 (ruler snapshot rows + the JSONL Nav flash mechanism), #317 (snapshot click → drawer headed with the title in effect + flash); verification #318 (the mockup's checks re-pointed at the real page with real data). #316 and #317 additionally depend on **#292**, the JSONL Nav itself, which does not exist on the real page yet.
