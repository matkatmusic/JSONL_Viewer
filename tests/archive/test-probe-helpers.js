// Tests for probe helper functions: temp file detection,
// NOT_TESTABLE status counting, and actionable pass rate.
...
run('test_countByStatus_includesNotTestable', function() {
  ...
  var counts = probe.countByStatus(results);
...
run('test_computeActionablePassRate_excludesNotTestable', function() {
  ...
  var rate = probe.computeActionablePassRate(counts);
...
run('test_buildNotTestable_returnsCorrectShape', function() {
  // Scenario: buildNotTestable produces the expected result object
  var r = probe.buildNotTestable('abc123.jsonl', 'foo.py', 'source JSONL deleted');
  assert.strictEqual(r.jsonlFile, 'abc123.jsonl');
  assert.strictEqual(r.filename, 'foo.py');
  assert.strictEqual(r.status, 'NOT_TESTABLE');
  assert.strictEqual(r.reason, 'source JSONL deleted');
  assert.strictEqual(r.totalEdits, 0);
...
run('test_parseProbeArgs_parsesSnapshotsFlag', function() {
  ...
  var opts = probe.parseProbeArgs(['--projects-dir', '/p', '--snapshots', '/snap/dir']);
...
run('test_parseProbeArgs_snapshotsDefaultsEmpty', function() {
  ...
  var opts = probe.parseProbeArgs(['--projects-dir', '/p']);
...
run('test_resolveSnapshotDir_explicitFlagWins', function() {
  ...
  var dir = probe.resolveSnapshotDir({ projectsDir: '/whatever', snapshots: '/explicit/snap' });
...
runWithContext('test_resolveSnapshotDir_autoDerivesSiblingFileHistoryWhenItExists', function(ctx) {
  ...
  var dir = probe.resolveSnapshotDir({ projectsDir: path.join(base, 'projects'), snapshots: '' });
  assert.strictEqual(dir, path.join(base, 'projects', '..', 'file-history'));
  assert.ok(fs.existsSync(dir));
});

// ─── cwdFromFolderName export (v2 refactor R1) ────────────────────────────

run('test_cwdFromFolderName_isExportedAndDecodesFolderName', function() {
  // Scenario: probe-projects exports cwdFromFolderName so probe-projects-v2 can
  // classify files by project root.
  // Step: decode a Claude projects folder name back to its original filesystem path.
  var result = probe.cwdFromFolderName('-Users-matkatmusicllc-Programming-jot');
