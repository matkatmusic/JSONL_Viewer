...
function renderDirNode(child, parent, depth, onFileSelect) {
  ...
  hdr.innerHTML = '<span class="tree-toggle">▾</span> ' +
    '<span class="tree-dir-name">' + escapeHtml(child.name) + '/</span>' +
    '<span class="tree-count">' + totalEdits + '</span>';
  dirEl.appendChild(hdr);

  var body = document.createElement('div');
  body.className = 'tree-dir-body';
  renderTreeNode(child, body, depth + 1, onFileSelect);
...
function renderFileNode(file, parent, depth, onFileSelect) {
  ...
  fileEl.innerHTML = '<span class="tree-file-name">' + escapeHtml(file.name) + '</span>' +
...
function renderTreeNode(node, parent, depth, onFileSelect) {
...
export function renderFileTree(fileMap, containerId, onFileSelect) {
  ...
  if (!container) { return; }
  container.innerHTML = '';
  renderTreeNode(root, container, 0, onFileSelect);
  requestAnimationFrame(function() { layoutTreePane(container); });
}

function layoutTreePane(treeBody) {
