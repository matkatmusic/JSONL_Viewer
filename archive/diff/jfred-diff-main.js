...
function showNumberedJson(lineIdx) {
  ...
  var highlighted = highlightJsonSyntax(pretty);
...
function updateDiffPanes(step, idx) {
  ...
  vsBaseEl.innerHTML = renderInlineDiff(computeLineDiff(baseLines, curLines));
  ...
  vsPrevEl.innerHTML = renderInlineDiff(computeLineDiff(prevLines, curLines));
