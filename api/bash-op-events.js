...
// representation). Mirrors originalFileEventsFromEdits — consumes the (frozen,
...
function rmEventsForOp(jsonlPath, parsed, op, aliasSet, cwd) {
  ...
  for (var i = 0; i < op.paths.length; i++) {
    ...
    events.push(createKindEvent(jsonlPath, op.line + 1, iso, 'bashRm', {}));
  }
  return events;
}
...
function redirectEventForOp(jsonlPath, parsed, op, aliasSet, cwd) {
  ...
  return [createKindEvent(jsonlPath, op.line + 1, iso, redirectKindForMode(op.mode), {})];
}

// Events for one bash op, by type (non-emitting types -> []).
function eventsForOp(jsonlPath, parsed, op, aliasSet, cwd) {
  if (op.type === 'rm') { return rmEventsForOp(jsonlPath, parsed, op, aliasSet, cwd); }
...
function bashOpEventsForFile(jsonlPath, jsonlText, parsed, aliasSet) {
  var cwd = extractSessionMetadata(jsonlText).cwd;
  var ops = extractBashFileOps(parsed);
  var events = [];
  for (var i = 0; i < ops.length; i++) {
    Array.prototype.push.apply(events, eventsForOp(jsonlPath, parsed, ops[i], aliasSet, cwd));
  }
  return events;
}

module.exports = {
  bashOpEventsForFile: bashOpEventsForFile
};
