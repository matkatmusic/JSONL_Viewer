...
// content-splitting + ref-pairing helpers via a CALL-time lse() getter (both
...
// each ref points at).
function chunkEntries(text, firstLine) {
  var lines = lse().splitContentLines(text);
  var spans = lse().contentLineSpans(text);
  var entries = [];
...
function materializeBashReadChunk(event) {
  var result = findToolResultText(loadParsedRecord(event.jsonl, event.jsonlLine));
  var entries = chunkEntries(result.text, event.bashReadChunk.firstLine);
  return { kind: 'bashReadChunk', byLine: lse().pairEntriesWithRefs(event, result.property, entries) };
}
...
// no-op extend).
function materializeBashExtent(event) {
  var result = findToolResultText(loadParsedRecord(event.jsonl, event.jsonlLine));
...
// any non-numbered row don't match and are skipped — modeled on numberedEntries.
...
function materializeBashGrep(event) {
  var result = findToolResultText(loadParsedRecord(event.jsonl, event.jsonlLine));
  return { kind: 'bashGrep', byLine: lse().pairEntriesWithRefs(event, result.property, grepEntries(result.text)) };
}

module.exports = {
  materializeBashReadChunk: materializeBashReadChunk,
  materializeBashExtent: materializeBashExtent,
  materializeBashGrep: materializeBashGrep
};
