...
// content property. Reuses line-state-evidence's ref pairing via a CALL-time lse() getter
...
function rowIsForFile(row, cwd, filePath) {
...
function materializeGrepMatches(event) {
  ...
  var rows = parseGrepRows(result.text).filter(function (row) {
    return rowIsForFile(row, event.grepMatches.cwd, event.grepMatches.filePath);
  });
  return { kind: 'grepMatches', byLine: lse().pairEntriesWithRefs(event, result.property, rows) };
}

module.exports = {
  materializeGrepMatches: materializeGrepMatches
};
