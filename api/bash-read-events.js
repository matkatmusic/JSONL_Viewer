...
// file-event-kinds, splitContentLines from line-state-evidence, parseBashReadCommand
...
var splitContentLines = require('./line-state-evidence').splitContentLines;
...
function buildSub(parsedCmd, lineCount) {
...
function emitResultEvent(item, record, resultIndex, ctx) {
  ...
  var lineCount = splitContentLines(stdout).length;
  if (lineCount === 0) { return null; }
  var iso = timestampAt(ctx.parsed, resultIndex);
  if (!iso) { return null; }
  return createKindEvent(ctx.jsonlPath, resultIndex + 1, iso, parsedCmd.kind, buildSub(parsedCmd, lineCount));
}
...
module.exports = {
  bashReadEventsForFile: bashReadEventsForFile
};
