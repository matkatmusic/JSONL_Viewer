...
function eventsForResult(jsonlPath, result, aliasSet, cwd) {
  var paths = distinctResolvedPaths(result.rows, cwd);
  var events = [];
  for (var i = 0; i < paths.length; i++) {
    if (!aliasSet.has(paths[i])) { continue; }
    events.push(createKindEvent(jsonlPath, result.resultLine, result.timestamp, 'grepMatches', { filePath: paths[i], cwd: cwd }));
  }
  return events;
}
...
function grepMatchEventsForFile(jsonlPath, jsonlText, parsed, aliasSet) {
  ...
  for (var i = 0; i < results.length; i++) {
    Array.prototype.push.apply(events, eventsForResult(jsonlPath, results[i], aliasSet, cwd));
  }
  return events;
}
