...
function lineDiff(a, b) {
  var dp = buildLcsDp(a, b);
  return traceLcs(a, b, dp);
}
...
function renderInlineDiff(ops) {
  ...
  for (var i = 0; i < ops.length; i++) {
    ...
    h += '<div class="diff-line ' + cls + '">' + _esc(pfx + o.text) + '</div>';
  }
  return h + '</div></div>';
}
...
function renderSxsChangeBlock(dels, adds) {
  ...
  for (var r = 0; r < rows; r++) {
    var left = r < dels.length ? '<div class="diff-cell del">' + _esc('-' + dels[r]) + '</div>' : '<div class="diff-cell empty"></div>';
    var right = r < adds.length ? '<div class="diff-cell add">' + _esc('+' + adds[r]) + '</div>' : '<div class="diff-cell empty"></div>';
...
function renderSxsDiff(ops) {
  ...
  while (i < ops.length) {
    if (ops[i].op === 'ctx') {
      h += '<div class="diff-sxs-row ctx-row"><div class="diff-cell ctx">' + _esc(' ' + ops[i].text) + '</div></div>';
      i++;
    } else {
