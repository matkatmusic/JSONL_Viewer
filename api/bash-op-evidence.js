...
// line-state-evidence and this module form a require CYCLE: its materializeEvent
...
function lse() { return require('./line-state-evidence'); }
...
function materializeBashRm(event) {
  ...
  var ref = lse().buildTextPropertyRef(event.jsonl, event.jsonlLine, property, 0, item.command.length);
...
function singleQuotedSpan(producer) {
...
function printfIsLiteral(content) {
...
function nonEmptySpan(span) {
  if (span.content === '') { return null; }
  return span;
}

function printfSpan(producer) {
  var span = singleQuotedSpan(producer);
  if (!printfIsLiteral(span.content)) { return null; }
  return nonEmptySpan(span);
}
...
function redirectContentFromCommand(command) {
  ...
  if (ECHO_SQ.test(producer)) { return nonEmptySpan(singleQuotedSpan(producer)); }
  if (PRINTF_SQ.test(producer)) { return printfSpan(producer); }
  return null;
}

// ─── Redirect materialization (truncate / append) ────────────────────────────

// Shift each plainLineEntries span by where the content sits in the command.
function offsetEntries(entries, base) {
...
function redirectByLine(event, item, span) {
  var property = 'message.content[' + item.index + '].input.command';
  var entries = offsetEntries(lse().plainLineEntries(span.content), span.startIndex);
  return lse().pairEntriesWithRefs(event, property, entries);
}
...
function materializeBashTruncate(event) {
  var record = loadParsedRecord(event.jsonl, event.jsonlLine);
  var item = findBashCommandItem(record);
...
function materializeBashAppend(event) {
  var record = loadParsedRecord(event.jsonl, event.jsonlLine);
  var item = findBashCommandItem(record);
  var span = redirectContentFromCommand(item.command);
...
module.exports = {
  ...
  materializeBashRm: materializeBashRm,
  materializeBashTruncate: materializeBashTruncate,
  materializeBashAppend: materializeBashAppend
};
