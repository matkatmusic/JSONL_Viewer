...
// line-state-evidence's materializeEvent dispatches here, and this module reuses
...
function contextEntry(event, cursor, line, hunkIndex, lineIndex) {
  var ref = lse().buildStructuredPatchRef(event.jsonl, event.jsonlLine, 'toolUseResult.structuredPatch', hunkIndex, lineIndex);
...
function appendHunkContext(byLine, event, hunk, hunkIndex) {
  ...
  for (var lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    byLine.push(contextEntry(event, cursor, lines[lineIndex], hunkIndex, lineIndex));
    cursor++;
  }
}
...
function materializePatchContext(event) {
...
module.exports = {
  materializePatchContext: materializePatchContext
};
