...
var unquotePath = require('./extract-bash-file-ops').unquotePath;
...
function readChunk(rawPath, firstLine, requested) {
  return { kind: 'bashReadChunk', path: unquotePath(rawPath), firstLine: firstLine, requested: requested };
}
...
function sedScriptGeometry(script) {
...
function parseSed(cmd) {
  ...
  if (!m) { return null; }
  var geom = sedScriptGeometry(unquotePath(m[1]));
  if (!geom) { return null; }
...
function parseWc(cmd) {
  var m = WC_PATTERN.exec(cmd);
  if (!m) { return null; }
  return { kind: 'bashExtent', path: unquotePath(m[1]) };
}
...
function parseGrep(cmd) {
  ...
  return { kind: 'bashGrep', path: unquotePath(rest[1]) };
}
...
function parseBashReadCommand(cmd) {
  ...
  return parseHead(trimmed) || parseSed(trimmed) || parseTail(trimmed) || parseWc(trimmed) || parseGrep(trimmed);
}
