...
// capture a stale export, so it is fetched at CALL time — mirrors resolver() in
// grep-tool-results.
function resolver() { return require('./file-historical-lineage').resolveAgainstCwd; }
...
function touchForResult(item, resultIndex, pending, cwd, resolveAgainstCwd) {
  if (!item) { return null; }
...
function collectBashReadTouches(parsed, cwd) {
  var resolveAgainstCwd = resolver();
  var pending = {};
  var touches = [];
  for (var i = 0; i < parsed.length; i++) {
    ...
    for (var c = 0; c < content.length; c++) {
      ...
      var touch = touchForResult(content[c], i, pending, cwd, resolveAgainstCwd);
