...
function indexObject(obj, lineIdx) {
...
function buildChips() {
  ...
  for (var p = 0; p < PRESET_FIELDS.length; p++) {
    ...
    chip.innerHTML = esc(field) + '<span class="chip-count' + (count > 0 ? ' has-matches' : '') + '">' + count + '</span>';
...
export function bindFilterEvents() {
  ...
  bar.addEventListener('click', function(e) {
    ...
    if (activeFilter === field) { nextMatch(); return; }
  ...
  document.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'n') { nextMatch(); e.preventDefault(); }
    if (e.key === 'N') { prevMatch(); e.preventDefault(); }
  });
}
