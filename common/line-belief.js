...
function overlayLine(belief, lineNum, text, ref, unixMs) {
...
function applyOverlayLines(belief, byLine, unixMs) {
  ...
  for (var b = 0; b < byLine.length; b++) {
    var conflict = overlayLine(belief, byLine[b].lineNum, byLine[b].text, byLine[b].ref, unixMs);
...
function finishChunk(belief, hitEof, lastCoveredLine) {
  if (!hitEof) { return; }
