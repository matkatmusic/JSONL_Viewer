...
export function renderFileContent(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    html += `<div class="pane-line"><span class="pane-line-num">${i + 1}</span>${esc(lines[i])}</div>`;
  }
  return html;
}
...
function renderDivergenceCheck(expected, actual, container, badge) {
  ...
  container.innerHTML = renderInlineDiff(lineDiff(expectedLines, actualLines));
