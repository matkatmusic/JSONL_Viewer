// ARCHIVED (phase 3) — entire module MOVED to api/file-path-history.js:
// buildFilePathHistoryIndex, findEarliestFilePath, findCurrentOnDiskPath,
// compareTouchOrder (and internal helpers). The commented-out original follows.
...
// // api/transcript-discovery.js and the lineage primitives in collect-touches.js
// // so the per-run scan lives in one place.
//
// var fs;
...
//     require('../api/transcript-discovery').loadAllJsonlFilesInProjectsFolder;
//   var ct = require('./collect-touches');
//   gatherAllOps = ct.gatherAllOps;
//   buildLineageGraph = ct.buildLineageGraph;
//   resolveAliases = ct.resolveAliases;
// }
//
...
// function earliestTouch(touches) {
...
// function allPathsForFile(knownFilePaths, index) {
...
//   var aliasPaths = allPathsForFile(knownFilePaths, index);
//   var earliest = null;
//   var earliestPath = knownFilePaths.length > 0 ? knownFilePaths[0] : '';
//   for (var a = 0; a < aliasPaths.length; a++) {
//     var candidate = earliestTouch(index.touchesByPath[aliasPaths[a]]);
...
//     var recency = touches ? latestTouch(touches) : { timestamp: null, line: 0 };
