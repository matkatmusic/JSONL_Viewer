// ARCHIVED (phase 3) — entire module MOVED to api/extract-bash-file-ops.js:
// unquotePath, parseBash{Cp,Mv,GitMv,Rm,Redirect}Command, extractBashFileOps
// (and internal helpers). The commented-out original follows.
...
//   return { src: unquotePath(match[1]), dst: unquotePath(match[2]) };
// }
//
...
//   var paths = argStr.split(/\s+/).map(unquotePath);
...
//   if (!m) { return null; }
//   return { type: 'redirect', path: unquotePath(m[2]), mode: m[1] };
// }
//
