...
2. Extracted open-work sections (every `### Open questions` section from implementation notes, the `## What Remains` section from handoffs; each under a `=== <file> ===` banner): !`sh .claude/skills/update-tasks/scripts/extractOpenSections.sh`
...
4. **Append new tasks to `tasks.json`** in the existing format:
   - `taskNumber` = highest number across both `tasks.json` and `completedTasks.json` plus one (increment for each new task)
   - `title` = short summary
   - `description` = the open item in the source file's own wording, with enough context to act on it later (file paths, item numbers)
   - `handoffFilePaths` = the source file's **archived** path, e.g. `plans/archived/implementation-notes-item66-fork-style-port.md`
   - Omit completion-related fields.

5. **Archive the processed files**: run `sh .claude/skills/update-tasks/scripts/archiveProcessed.sh`. It moves every file from step 1 into `plans/archived/` (a file that yielded no new tasks is still retired by processing it) and leaves any file in place whose name already exists in `plans/archived/`, printing `COLLISION` for it — report those collisions.
