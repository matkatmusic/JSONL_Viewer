...
function alLineLabel(obj, idx) {
  var st = alSubtype(obj);
  if (st === 'user-prompt') { return '<span class="al-user">User:</span> ' + esc(alContentLabel(obj)); }
  if (st === 'assistant-text') { return '<span class="al-agent">Agent:</span> ' + esc(alContentLabel(obj)); }
  if (st === 'thinking') { return '<span class="al-dim">Thinking</span>'; }
  if (st === 'tool-use') { return '<span class="al-tool">tool_use: ' + esc(alToolName(obj)) + '</span>'; }
  ...
  if (st === 'preamble') { return '<span class="al-dim">' + esc(obj && obj.type || 'preamble') + '</span>'; }
  if (st === 'attach') { return '<span class="al-dim">attachment</span>'; }
  return '<span class="al-dim">L' + (idx + 1) + '</span>';
}

function alSegLabel(obj, idx) {
  var st = alSubtype(obj);
  if (st === 'user-prompt') { return '<span class="al-user">User:</span> ' + esc(alContentLabel(obj)); }
  if (st === 'assistant-text') { return '<span class="al-agent">Agent:</span> ' + esc(alContentLabel(obj)); }
  return alLineLabel(obj, idx);
}
...
function alBuildSegments() {
  var segments = [];
  var parsed = state.parsedLines;
  var convIndices = [];
  for (var i = 0; i < parsed.length; i++) {
    if (isConv(alSubtype(parsed[i]))) { convIndices.push(i); }
  }
...
function alRenderRow(lineIdx) {
  ...
  return '<div class="' + cls + '" data-alidx="' + lineIdx + '">' +
    ...
    alLineLabel(obj, lineIdx) + badge + '</div>';
}

function alSegEditBadges(lines) {
  var edits = {};
  for (var i = 0; i < lines.length; i++) {
...
function alRenderSegment(seg) {
  ...
  for (var i = 0; i < all.length; i++) { html += alRenderRow(all[i]); }
  html += '</div>';
  return html;
}

function alRenderSegments(segs) {
  var html = '';
  for (var s = 0; s < segs.length; s++) { html += alRenderSegment(segs[s]); }
  return html;
}

function alRenderBranch(label, cls, segs, open) {
  ...
  html += alRenderSegments(segs) + '</div>';
  return html;
}
...
export function renderAllLines() {
  ...
  var segs = alBuildSegments();
  var rwSegs = [];
  for (var s = 0; s < segs.length; s++) {
    if (segs[s].rw) { rwSegs.push(s); }
  }
  if (rwSegs.length === 0) {
    el.innerHTML = alRenderSegments(segs);
    return;
  }
  el.innerHTML = alRenderWithBranches(segs, rwSegs);
}
...
function alRenderWithBranches(segs, rwSegs) {
  var html = '';
  var cursor = 0;
  var bn = 0;
  for (var b = 0; b < rwSegs.length; b++) {
    ...
    if (fork >= cursor) {
      html += alRenderSegments(segs.slice(cursor, fork + 1));
      cursor = fork + 1;
    }
    ...
    if (old.length > 0) {
      html += alRenderBranch('old branch (' + old.length + ' turns)', 'al-br-inactive', old, false);
    }
    html += alRenderBranch('active branch (' + active.length + ' turns)', 'al-br-active', active, true);
    cursor = nextRw;
  }
  if (cursor < segs.length) { html += alRenderSegments(segs.slice(cursor)); }
  return html;
}
