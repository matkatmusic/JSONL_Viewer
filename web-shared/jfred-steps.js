...
function stepRowHtml(step, idx) {
  ...
  return `<div class="${classes}" data-step="${idx}">` +
    ...
    `<span class="${typeCls}">${esc(step.type)}</span>` +
    `<span class="${srcCls}">${esc(src)}</span>${lineHtml}</div>`;
}
...
export function renderSteps() {
  ...
  for (let i = 0; i < state.steps.length; i++) {
    html += stepRowHtml(state.steps[i], i);
  }
  el.innerHTML = html;
}
...
function updateInspector() {
  ...
  jsonEl.innerHTML = syntaxHighlight(JSON.stringify(inspectObj, null, 4));
}
