...
Then, create a monitoring loop that checks the `plans/` folder for when the handoff document for Scenario X is created.  When the handoff document lands, begin implementing the plan for handling Scenario X using the /jot:implement skill.  When the implementation of the handling of Scenario X is finished, write a handoff doc using the /jot:handoff-prompt skill.
